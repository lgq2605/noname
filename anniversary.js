...
if (isRecord) window.annivCues = [];
let recT0 = 0;
function cue(name){ if (isRecord && recT0) window.annivCues.push({ cue: name, t: (performance.now() - recT0) / 1000 }); }
...
function treeFrame(now){
  ...
  if (!window.annivDone && t >= T.done) window.annivDone = true;
...
function treeStart(){
  treeStartT = 0; treeLastT = 0; lastPetal = 0; replayArmed = false; window.annivDone = false;
  cue('grow');
  buildScene();
...
function resetAll(){
  treeStop();
  showWish(false);
  window.annivDone = false; replayArmed = false;
